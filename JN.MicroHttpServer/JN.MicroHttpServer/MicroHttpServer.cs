...
namespace JN.MicroHttpServer
{
    public class MicroHttpServer : IMicroHttpServer
    {
        ...
        public IEnumerable<string> UriPrefixes { get; set; }
        ...
        public bool IsInitialized => UriPrefixes != null && UriPrefixes.Any();
        ...
        private async Task StartListener(CancellationToken token)
        {
            ...
            try
            {
                ...
                if (UriPrefixes == null || !UriPrefixes.Any())
                {
                    throw new ArgumentException("Invalid base URLs list");
                }

                foreach (var prefix in UriPrefixes)
                {
                    listener.Prefixes.Add($"{prefix}/status/");
                    WriteOutput($"URL: {prefix}/status/");

                    listener.Prefixes.Add($"{prefix}/shutdown/");
                    WriteOutput($"URL: {prefix}/shutdown/");
                }
